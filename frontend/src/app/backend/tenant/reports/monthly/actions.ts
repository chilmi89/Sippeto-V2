...
export async function getMonthlyReportData(selectedBranchId = "all") {
  try {
    ...
    // 3. Ambil data penjualan (monthly) dari Golang
    ...
    if (!reportRes.ok) {
      return { status: "error", message: "Gagal memuat laporan penjualan bulanan dari server Go" };
    }
    ...
    console.error("Server Action getMonthlyReportData Error:", error);
    return {
      status: "error",
