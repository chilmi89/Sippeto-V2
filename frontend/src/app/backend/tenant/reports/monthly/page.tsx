...
export default async function MonthlyReportPage({ searchParams }: PageProps) {
  ...
  const reportResult = await getMonthlyReportData(selectedBranchId);
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-4 bg-white/80 backdrop-blur-md px-5 py-3.5 rounded-2xl border border-zinc-100 shadow-sm self-start sm:self-center">
          ...
          <div className="flex items-center gap-3 w-full sm:w-auto justify-between sm:justify-start">
            ...
            <div className="flex items-center gap-2">
              <Link
                href={`/api/backend/reports/export?type=monthly&format=excel&branch_id=${selectedBranchId}`}
              ...
              <Link
                href={`/api/backend/reports/export?type=monthly&format=pdf&branch_id=${selectedBranchId}`}
