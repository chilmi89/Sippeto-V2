...
export default async function MonthlyReportPage({ searchParams }: PageProps) {
  ...
  const reportResult = await getMonthlyReportData(
    selectedBranchId,
    dateStart,
    dateEnd,
    sortOrder
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 py-2">
        <div className="max-w-xl">
          ...
          <p className="text-zinc-500 font-medium text-sm mt-3">
            Analisis data penjualan bulanan dan aktivitas finansial Anda secara
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-4 bg-white/80 backdrop-blur-md px-5 py-3.5 rounded-2xl border border-zinc-100 shadow-sm self-start sm:self-center">
          ...
          <div className="flex items-center gap-3 w-full sm:w-auto justify-between sm:justify-start">
            ...
            <div className="flex items-center gap-2">
              <Link
                href={`/api/backend/reports/export?type=monthly&format=excel&branch_id=${selectedBranchId}${dateStart ? `&date_start=${dateStart}` : ''}${dateEnd ? `&date_end=${dateEnd}` : ''}&sort=${sortOrder}`}
              ...
              <Link
                href={`/api/backend/reports/export?type=monthly&format=pdf&branch_id=${selectedBranchId}${dateStart ? `&date_start=${dateStart}` : ''}${dateEnd ? `&date_end=${dateEnd}` : ''}&sort=${sortOrder}`}
        ...
        <div className="flex flex-col xs:flex-row items-start xs:items-center justify-between gap-4 mb-6 relative z-10">
          <div>
            ...
            <p className="text-xs text-zinc-400 font-medium">
              Perbandingan pemasukan dan pengeluaran bulanan (Server-rendered SVG)
            </p>
          </div>
          <div className="flex items-center gap-2">
        ...
        <div className="w-full overflow-x-auto">
          <div className="min-w-[760px] h-[280px]">
            <svg
              ...
              role="img"
              aria-label="Grafik Kinerja Arus Kas Bulanan"
            >
              <title>Grafik Kinerja Arus Kas Bulanan</title>
              <defs>
